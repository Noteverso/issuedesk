...
- Q: When the app is offline and users make multiple changes (create/edit/delete issues and labels), how should these changes be synced when connectivity returns? → A: Sequential sync with conflict detection - prompt user to review conflicts before applying
...
- Q: The app needs local storage for offline functionality. Which database solution should be used? → A: SQLite - embedded relational database with full SQL support
...
- When GitHub API rate limit is exhausted, system MUST display warning when remaining requests fall below 20% of hourly limit, queue pending operations locally, and automatically retry queued operations after rate limit reset time
- When an issue is modified both locally and on GitHub simultaneously, system MUST detect the conflict and display a conflict resolution dialog showing both versions (local and remote) with options to: keep local changes, accept remote changes, or manually merge the differences
- When a comment is modified both locally and on GitHub simultaneously, system MUST detect the conflict and display a conflict resolution dialog showing both versions (local and remote) with options to: keep local changes, accept remote changes, or manually merge the differences
...
- **FR-014a**: System MUST maintain separate data cache (SQLite database) for each repository configured by user
...
- **FR-016**: System MUST store all data locally using one SQLite database per repository, with tables for issues, comments, labels, sync queue, and repository metadata
- **FR-017**: System MUST sync local changes with GitHub repository when online using sequential processing of queued operations (including comment CRUD operations)
- **FR-017a**: System MUST detect conflicts when both local and remote versions of an issue/label have been modified
- **FR-017b**: System MUST prompt user to resolve conflicts before completing sync, displaying both versions for comparison
...
- **FR-019c**: System MUST queue operations locally when rate limit exhausted and auto-retry after reset
- **FR-020**: System MUST display error messages when operations fail

### Key Entities

- **Issue**: Represents a GitHub issue with title, body (markdown), labels (full Label objects with all properties for rich UI display), state (open/closed), created/updated timestamps, GitHub URL, local sync status, comment count, and repository identifier
- **Comment**: Represents a GitHub issue comment with markdown body containing embedded HTML comment metadata (title, description, tags), author information, created/updated timestamps, issue ID reference, local sync status, and repository identifier. HTML metadata format:
  ```html
  <!-- title: Comment Title -->
...
- **SyncQueue**: Tracks pending operations to sync with GitHub when online (create, update, delete for issues, comments, and labels), maintains operation order, includes conflict detection metadata (local/remote timestamps and checksums), stores rate-limit queued operations with scheduled retry time, scoped per repository
...
- **SC-003**: App remains functional offline with all core features (viewing, editing, creating) available using local data
- **SC-004**: Filter operations return results within 100ms for repositories with up to 1000 issues using SQLite indexed queries
...
- **SC-007**: Dashboard analytics display accurate counts matching GitHub repository state after sync, with trend calculations based on issue created/updated timestamps
